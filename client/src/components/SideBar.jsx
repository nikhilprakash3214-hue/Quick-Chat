...
const SideBar = () => {
    ...
    const { logout, onlineUsers, userData } = useContext(AuthContext);
    ...
    user._id !== userData?._id && // Hide yourself
    user.fullName.toLowerCase().includes(input.toLowerCase())
);

//     // Provide a fallback empty array [] so .map() doesn't fail
//     const filteredUsers = (users || []).filter((user) => 
...
// );
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="relative py-2 group">
                        ...
                        transition-all">
                            ...
                            text-sm hover:text-purple-400">Edit Profile</p>
                            <hr className="my-2 border-t border-gray-500" />
                            <p onClick={() => logout()} className="cursor-pointer text-sm
                             hover:text-red-400">Logout</p>
                        </div>
                    </div>
                </div>
 ...
 {/* Search Bar */}
     ...
     border border-transparent focus-within:border-purple-500/50 transition-all">
     ...
     <div className='flex flex-col'>
                {filteredUsers.map((user, index) => (
                    <div onClick={()=> {setSelectedUser(user);
                        ...
                        key={user._id} className={`relative flex items-center gap-2 pl-4 
